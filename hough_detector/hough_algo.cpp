...
void HoughDetector::solve() {
    ...
    while (true) {
        ...
        cv::putText(m_all_lines, std::to_string(row_num), Point2i(0, cur_y), 1, 1, Scalar(255 ,0, 0));
        ...
        for (int q = 0; q < cur_lines.size(); q++){
            cv_supp::Line line = cur_lines[q];
            ...
            for (CellPtr prev_line_as_cell : prev_lines_as_cells) {
                ...
                int approximate_diff = 1;//get_size_by_y(m_min_approximate_diff, m_max_approximate_diff, cur_y);
                if (abs(cur_line_as_cell->line.cartesLine.max.x - prev_line_as_cell->line.cartesLine.min.x) < approximate_diff) {
    ...
    for (int i = 0; i < m_grid.size() - 2; i++) {
        for (int cell_idx = 0; cell_idx < m_grid[i].size(); cell_idx++) {
            ...
            plot = false;
            if (cur_cell->line.cartesLine.max.x == 330 && cur_cell->line.cartesLine.max.y == 116)
            {
                plot = true;
            }

            if (!cur_cell->has_same_direction(cur_cell->parents))
                continue;
...
std::vector<cv_supp::Line> HoughDetector::get_lines_on_cropped(int y_min, int y_max) {
    ...
    HoughLines(cur_canny_image, lines, 1, CV_PI / 180, (y_max - y_min) * 2 / 3, 0, 0, -CV_PI / 3, CV_PI / 3);
...
/*
    ...
    for (int i = 0; i < cur_size; i++) {
        ...
        for (int j = i + 1; j < cur_size; j++){
            ...
            int approximate_diff = get_size_by_y(m_min_approximate_diff, m_max_approximate_diff, line_i.max.y);
            if (math_utils::get_diff(line_i.max.x, line_j.max.x) < approximate_diff && math_utils::get_diff(line_i.min.x, line_j.min.x) < approximate_diff) {
                same_lines_count++;
                res_x1 += line_j.max.x;
                res_x2 += line_j.min.x;
...
bool HoughDetector::is_intersection_neighs(CellPtr c1, CellPtr c2, int same_direction_depth, int neighs_check_depth, CellPtr came_from1, CellPtr came_from2) {
    ...
    if (plot) {
        c1->line.print_cartes();
        c2->line.print_cartes();
    }
    ...
    if (HoughDetector::Cell::is_different_direction_lines(c1, c2)) {
        ...
        if (both_has_same_direction_neighs) {
            ...
            {
                cv::line(m_all_lines, c1->line.cartesLine.min, c1->line.cartesLine.max, Scalar(0, 255, 255));
                cv::line(m_all_lines, c2->line.cartesLine.min, c2->line.cartesLine.max, Scalar(0, 255, 255));
                return true;
...
bool HoughDetector::Cell::is_different_direction_lines(std::shared_ptr<HoughDetector::Cell> c1, std::shared_ptr<HoughDetector::Cell> c2) {
    ...
    bool res = cos_diff > m_not_parallel_cos_diff;
    if (plot)
        std::cout << "diff_dir cos = " << cos_diff << ", res: " << res << std::endl;

    return res;
}

bool HoughDetector::is_same_direction_lines(cv_supp::Line l1, cv_supp::Line l2, double same_dir_diff) {
    float cos_diff =  math_utils::get_diff(cv_supp::get_line_cos(l1.cartesLine.max, l1.cartesLine.min),
                                           cv_supp::get_line_cos(l2.cartesLine.max, l2.cartesLine.min));

    bool res = cos_diff < same_dir_diff;
    if (plot)
        std::cout << "same_dir cos = " << cos_diff << ", res: " << res <<  std::endl;
